...
void GLGenericDrawable::AddElem( const TooN::Vector<> &attrib0 ) {
	GLfloat *elem = PushElem();
	for ( GLuint i = 0; i < attribSizes[0]; i++,elem++ ) *elem = attrib0[i];
}

void GLGenericDrawable::AddElem( const TooN::Vector<> &attrib0, const TooN::Vector<> &attrib1 ) {
	GLfloat *elem = PushElem();
	for ( GLuint i = 0; i < attribSizes[0]; i++,elem++ ) *elem = attrib0[i];
	for ( GLuint i = 0; i < attribSizes[1]; i++,elem++ ) *elem = attrib1[i];
}

void GLGenericDrawable::AddElem( const TooN::Vector<> &attrib0, const TooN::Vector<> &attrib1, const TooN::Vector<> &attrib2 ) {
	GLfloat *elem = PushElem();
	for ( GLuint i = 0; i < attribSizes[0]; i++,elem++ ) *elem = attrib0[i];
	for ( GLuint i = 0; i < attribSizes[1]; i++,elem++ ) *elem = attrib1[i];
...
void GLGenericDrawable::AddElem( const TooN::Vector<> &attrib0, const TooN::Vector<> &attrib1, const TooN::Vector<> &attrib2, const TooN::Vector<> &attrib3 ) {
	GLfloat *elem = PushElem();
	for ( GLuint i = 0; i < attribSizes[0]; i++,elem++ ) *elem = attrib0[i];
	for ( GLuint i = 0; i < attribSizes[1]; i++,elem++ ) *elem = attrib1[i];
	for ( GLuint i = 0; i < attribSizes[2]; i++,elem++ ) *elem = attrib2[i];
...
void GLGenericDrawable::SetElem( size_t index, const TooN::Vector<> &attrib0 ) {
	GLfloat *elem = data + elemSize * index;
	for ( GLuint i = 0; i < attribSizes[0]; i++,elem++ ) *elem = attrib0[i];
}

void GLGenericDrawable::SetElem( size_t index, const TooN::Vector<> &attrib0, const TooN::Vector<> &attrib1 ) {
	GLfloat *elem = data + elemSize * index;
	for ( GLuint i = 0; i < attribSizes[0]; i++,elem++ ) *elem = attrib0[i];
	for ( GLuint i = 0; i < attribSizes[1]; i++,elem++ ) *elem = attrib1[i];
}

void GLGenericDrawable::SetElem( size_t index, const TooN::Vector<> &attrib0, const TooN::Vector<> &attrib1, const TooN::Vector<> &attrib2 ) {
	GLfloat *elem = data + elemSize * index;
	for ( GLuint i = 0; i < attribSizes[0]; i++,elem++ ) *elem = attrib0[i];
	for ( GLuint i = 0; i < attribSizes[1]; i++,elem++ ) *elem = attrib1[i];
	for ( GLuint i = 0; i < attribSizes[2]; i++,elem++ ) *elem = attrib2[i];
